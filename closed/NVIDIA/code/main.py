...
from code.common.mps import ScopedMPS, turn_off_mps
from code.common.power_limit import ScopedPowerStateController, get_power_controller
...
def handle_run_harness(config, gpu=True, dla=True, profile=None,
    ...
    # Dump additional stats that are useful for engineers
    # 1) List comparison of the perf stats against the current perf stored in results/ folder
    # 2) If Server, list 99-percentile latency / threshold, and server qps / offline qps
    if not compliance and not power:
        if "test_mode" not in config or config["test_mode"] != "AccuracyOnly":
            print(f"\n======================= Extra Perf Stats: =======================\n")
            print(f"{config['config_name']}-{config['config_ver']}:")
            log_paths = glob.glob(os.path.join(log_dir, "**", "mlperf_log_detail.txt"), recursive=True)
            if len(log_paths) > 1:
                print(f"    More than one result found under {log_dir}, Skipping the extra stats...")
            else:
                ...
                # lazy import the module and query the loadgen logs
                from code.common.log_parser import read_loadgen_result_by_key, scenario_loadgen_log_keys, from_loadgen_by_keys, get_perf_regression_ratio
                ...
                assert scenario_for_metric == config["scenario"]
                scenario_key = scenario_loadgen_log_keys[Scenario.get_match(scenario_for_metric)]
                perf_number = read_loadgen_result_by_key(log_dir, scenario_key, None)
                regression_ratio, current_perf = get_perf_regression_ratio(perf_number, "results", system_id, benchmark, scenario)
                if current_perf != 0.0:
                    print(f"    {scenario_key}: {perf_number:.2f} is {regression_ratio:.2f} of the current results {current_perf:.2f}.")
                else:
...
def check_accuracy(log_file, config, is_compliance=False):
    ...
    new_path = [os.path.join(os.getcwd(), "build", "inference", "tools", "submission")] + sys.path
    with ScopedRestrictedImport(new_path):
    ...
    if benchmark in [Benchmark.ResNet50]:
        ...
        if SystemClassifications.is_soc():
            ...
            pythonpath_extra = ["code/bert/tensorrt/helpers"]
        ...
        cmd = "python3 code/3d-unet/tensorrt/accuracy_kits.py --log_file {:}".format(log_file)
        regex = accuracy_regex_map["DICE"]
        # Cannot use nibabel in SoC systems so cannot check the accuracy directly
        if SystemClassifications.is_soc():
            ...
            try:
                ...
                logging.warning(
                    "Accuracy checking for 3D-UNet is not supported on SoC systems. "
    ...
    if not skip_run_command:
        cmd = fix_pythonpath_command(cmd, pythonpath_extra=pythonpath_extra)
        if len(cmd_prefix) > 0:
            cmd = cmd_prefix + " " + cmd
...
def dispatch_action(main_args, benchmark_conf, conf_ver, equiv_engine_setting=None):
    ...
    if action == "generate_engines":
        with ScopedPowerStateController(power_controller):
...
if __name__ == "__main__":
    ...
    if MATCHED_SYSTEM is None:
        ...
        mismatches, num_mismatches = DETECTED_SYSTEM.verify_system_json()
        if num_mismatches != 0:
            logging.warning(f"Keys in System JSONs inconsistent with detected hardware: {mismatches}")
