...
from code.common.constants import Benchmark, G_HIGH_ACC_ENABLED_BENCHMARKS
...
plugin_map = {
    # TODO: NMSOptPlugin doesn't support RetinaNet yet.
    Benchmark.Retinanet: [
                          # "build/plugins/NMSOptPlugin/libnmsoptplugin.so",
                          "build/plugins/retinanetConcatPlugin/libretinanetconcatplugin.so",
                         ],
    Benchmark.DLRM: ["build/plugins/DLRMInteractionsPlugin/libdlrminteractionsplugin.so"],
    Benchmark.SSDMobileNet: ["build/plugins/NMSOptPlugin/libnmsoptplugin.so"],
    Benchmark.SSDResNet34: ["build/plugins/NMSOptPlugin/libnmsoptplugin.so"],
    Benchmark.UNET3D: ["build/plugins/pixelShuffle3DPlugin/libpixelshuffle3dplugin.so",
                       "build/plugins/conv3D1X1X1K4Plugin/libconv3D1X1X1K4Plugin.so",
                       "build/plugins/conv3D3X3X3C1K32Plugin/libconv3D3X3X3C1K32Plugin.so"],
}

benchmark_qsl_size_map = {
    # See: https://github.com/mlcommons/inference_policies/blob/master/inference_rules.adoc#benchmarks-1
    Benchmark.BERT: 10833,
    Benchmark.DLRM: 204800,
    Benchmark.RNNT: 2513,
    Benchmark.ResNet50: 2048,
    Benchmark.Retinanet: 64,
    Benchmark.SSDMobileNet: 1024,
    Benchmark.SSDResNet34: 64,
    Benchmark.UNET3D: 43,
}
...
class BaseBenchmarkHarness:
    ...
    def __init__(self, args, benchmark, skip_file_checks=False):
        self.args = args
    ...
    def get_system_name(self, add_trt=True):
        ...
        if add_trt:
            system_name = "{:}_TRT{:}".format(system_name, TENSORRT_VERSION)
