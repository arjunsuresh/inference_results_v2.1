...
class RunHarnessHandler(GenerateConfFilesHandler):
    ...
    def __init__(self, benchmark_conf, power_controller, use_gpu=True, use_dla=True, profiler=None, measure_power=False):
        ...
        super().__init__(benchmark_conf, use_gpu=use_gpu, use_dla=use_dla)
    ...
    def setup(self):
        ...
        # We can't call super() here because super().run() will call self.setup() and self.handle(), which will be the
        # implementations for RunHarnessHandler, and cause an infinite recursion when self.setup() (this method) is called.
        # Cast to a GenerateConfFilesHandler instead temporarily.
        self.__class__ = GenerateConfFilesHandler
        self.run()
        self.__class__ = RunHarnessHandler
    ...
    def report_stats(self):
        ...
        if len(log_paths) > 1:
            ...
            assert scenario_for_metric == self.scenario
